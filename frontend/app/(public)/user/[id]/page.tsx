...
import { getUserById, getUserProfile } from "@/lib/api";
...
export default async function UserProfilePage(props: {
  ...
  // If the user tries to view their own profile via this route, redirect them to /profile
  if (currentUser && currentUser.id.toString() === params.id) {
    redirect("/profile");
  }

  let user = null;
  try {
    const res = await getUserById(token || "", parseInt(params.id));
    user = res.data;
  } catch (error) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center overflow-y-auto pb-10 scrollbar-hide w-full h-full">
      <div className="w-full max-w-3xl px-4 py-8">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-gray-700 hover:text-black hover:bg-gray-100 px-3 py-2 rounded-lg transition-colors font-medium mb-6"
        >
